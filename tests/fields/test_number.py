...
import jsonschema_formful.validators
from jsonschema_formful.field import NumberParameters
...
def test_max():
    ...
    hamcrest.assert_that(constraints, hamcrest.has_entries({
        'validators': hamcrest.contains_exactly(
            ...
            hamcrest.instance_of(jsonschema_formful.validators.NumberRange)
        )
    }))

    assert field.required is True
    assert field.get_factory() == formful.fields.IntegerField
...
def test_min():
    ...
    assert field.get_factory() == formful.fields.FloatField
    form = formful.form.BaseForm({"test": field()})
